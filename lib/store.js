...
import { initialBanners, initialFeatured, initialCategories, initialCoupons, initialOrders, initialSettings, initialExpenses, initialReminders, initialWaiterRequests } from './initialData';
...
function initializeMemoryStore() {
  ...
  memoryStore.set('waiterRequests', JSON.parse(JSON.stringify(initialWaiterRequests)));
...
function getDefaultData(key) {
  const defaults = {
    ...
    waiterRequests: initialWaiterRequests,
    sessions: []
  };
  return defaults[key];
}
